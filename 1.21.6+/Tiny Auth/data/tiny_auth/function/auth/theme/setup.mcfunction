...
$execute if predicate {condition:"minecraft:value_check",value:$(theme_id),range:-1} run data modify storage tiny_auth:temp themeSet set from storage tiny_auth:storage auths[{UUID:$(UUID)}].CustomTheme
