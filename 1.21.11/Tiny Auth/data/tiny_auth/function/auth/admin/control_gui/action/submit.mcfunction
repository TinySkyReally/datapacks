...
$execute if score @s tinyauth.auth.page matches 4..5 store result score #testnumber tinyauth.auth.temp run function tiny_auth:auth/enter/macro2 with storage tiny_auth:keys auths[{UUID:$(UUID)}]
...
$execute if score @s tinyauth.auth.page matches 4..5 unless score #testnumber tinyauth.auth.temp matches 0 run return run function tiny_auth:auth/admin/control_gui/action/submit/macro with storage tiny_auth:keys auths[{UUID:$(UUID)}]
