...
$execute if data storage tiny_auth:temp {plrData:{UUID:$(UUID)}} unless data storage tiny_auth:temp plrData.Name in tiny_auth:authdim run item replace block 1000 0 0 container.0 with minecraft:air
...
$execute if data storage tiny_auth:temp {plrData:{UUID:$(UUID)}} unless data storage tiny_auth:temp plrData.Name in tiny_auth:authdim run data modify storage tiny_auth:keys auths[{UUID:$(UUID)}].Name set from block 1000 0 0 Items[{Slot:0b}].components."minecraft:profile".name
