...
$execute if score @s tinyauth.auth.page matches -1 store result score @s tinyauth.auth.page run data get storage tiny_auth:storage auths[{UUID:$(UUID)}].prevpage

$execute store result storage tiny_auth:storage auths[{UUID:$(UUID)}].prevpage int 1 run scoreboard players get @s tinyauth.auth.page
...
$data modify storage tiny_auth:temp control_panel set from storage tiny_auth:storage auths[{UUID:$(UUID)}]
